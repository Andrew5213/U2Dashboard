...
**Schema change requires manual migration**: `observacoes` is a new column on the existing `clickup_task_cache` table. Since `Base.metadata.create_all` only creates missing tables and never alters existing ones (see Database section), this column will not appear automatically in an already-existing `sync.db`/`dev.db` — either delete the file and let it recreate (losing all cached data until the next refresh) or run `ALTER TABLE clickup_task_cache ADD COLUMN observacoes TEXT;` manually against the existing database.
